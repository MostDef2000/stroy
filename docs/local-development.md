...
STOY talks to Qwen via an OpenAI-compatible endpoint configured by:
...
ComfyUI is the initial image runtime. Workflows live under workflows/ and must have STOY-owned manifests defining stable inputs/outputs. Business logic must not depend on ComfyUI node IDs.

## Blender

The renderer invokes Blender headlessly via STOY_BLENDER_BIN. Blender scripts belong under blender/.
